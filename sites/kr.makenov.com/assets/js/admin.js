...
/* ============================================================
   2-B. 공급사(기업) 관리 — 제품의 company_id 가 여기를 가리킨다.
   기업정보 입력·연결 입구가 아예 없던 것을 신설 (2026-08-26 지시)
   ============================================================ */
let coEditing = null;
...
function renderCompanies(){
  ...
  if(!el) return;
  if(coEditing !== null){ el.innerHTML = companyForm(coEditing); return; }
  const prodCnt = id => MK_PRODUCTS.filter(p=>p.companyId===id).length;
  el.innerHTML = `
    <div class="card"><p class="note">제조사·브랜드의 <b>기업정보</b>입니다. 제품 수정 화면의 <b>공급사 연결</b>에서 이 목록을 고르면
    제품 상세페이지에 회사 소개·회사 페이지 링크가 붙습니다.</p><div class="bar"><span class="grow"></span><button class="btn btn-primary btn-sm" onclick="coEditing='';renderCompanies()">+ 새 공급사 등록</button></div><div class="tbl-wrap"><table><thead><tr><th style="width:56px">로고</th><th>회사명</th><th>브랜드</th><th>카테고리</th><th>소재지</th><th>연결 제품</th><th style="width:60px">순서</th><th style="width:120px"></th></tr></thead><tbody>${MK_COMPANIES.length ? MK_COMPANIES.map(c=>`
        <tr class="row-hover"><td><img class="thumb-sm" src="${esc(imgSrc(c.logo||''))}" alt="" onerror="this.style.visibility='hidden'"></td><td><b>${esc(triText(c.name))}</b><div class="sub">${esc(c.id)}</div></td><td>${esc(c.brand||'')}</td><td>${esc(mkCat(c.cat)?mkCat(c.cat).name.ko:(c.cat||''))}</td><td>${esc(triText(c.location))}</td><td><b>${prodCnt(c.id)}</b>개</td><td>${c.sort??''}</td><td><button class="btn btn-ghost btn-sm" onclick="coEditing='${c.id}';renderCompanies()">수정</button><button class="btn btn-ghost btn-sm" onclick="if(confirm('${esc(triText(c.name))}\\n삭제할까요? 이 회사에 연결된 제품 ${prodCnt(c.id)}개는 연결이 풀립니다.')){admDo(Admin.deleteCompany('${c.id}'));}">삭제</button></td></tr>`).join('') : `<tr class="empty-row"><td colspan="8">등록된 공급사가 없습니다</td></tr>`}
      </tbody></table></div></div>`;
}

function companyForm(id){
  const c = id ? MK_COMPANIES.find(x=>x.id===id) : null;
  const g = (o,k)=> (o && o[k]) ? o[k] : '';
  const nm=c?c.name:{}, tg=c?c.tagline:{}, it=c?c.intro:{}, lc=c?c.location:{};
  const tri3 = (base, label, val, kind) => `<div class="sect"><h4>${label} (3개 국어)</h4><div class="fgrid">${['ko','vi','en'].map(l=>{
    const tag = `<label><span class="lang-tag">${l.toUpperCase()}</span>${l==='ko'?'한국어':l==='vi'?'베트남어':'영어'}</label>`;
...
async function saveProduct(id){
  ...
  const p = {
    ...
    cat: av('f-cat'), companyId: av('f-company'),
    featured: ac('f-featured'), isNew: ac('f-new'), published: ac('f-pub'),
    ...
    video: av('f-video'),
    catalog: (document.getElementById('f-catalog-hide') && document.getElementById('f-catalog-hide').checked ? 'hide:' : '') + av('f-catalog'),
...
/* ============================================================
   제품별 '유통 파트너 모집' 섹션 (products.dist)
   ------------------------------------------------------------
   기본은 공통 문구(제품 탭 맨 위 카드)를 그대로 쓴다. 제품 폼에서
   '이 제품만 따로' 를 고르면 제목·✅ 항목을 세 언어로 따로 쓰고,
   '숨김' 이면 그 제품에서만 섹션이 빠진다. 폼을 다시 그리지 않고
   행을 DOM 에 바로 넣고 빼므로 다른 칸 입력이 안 날아간다.
   ============================================================ */
let pdDistSeq = 0;
function pdDistRowHtml(it){
  const k = pdDistSeq++;
  const o = it || {};
  return `<div class="blk" data-dist-row="${k}"><div class="blk-head"><b style="font-size:13px">✅ 항목</b><button class="btn btn-ghost btn-sm" onclick="this.closest('[data-dist-row]').remove()">삭제</button></div>
...
function pdDistFormHtml(p){
  ...
  return `<div class="sect"><h4>유통 파트너 모집 섹션 <span style="color:var(--adm-sub);font-size:11px;font-weight:500">갤러리와 제품 상세 사이 · 이 제품에만 적용</span></h4>
    ...
    <div id="f-dist-box">
      ...
      <div id="f-dist-items">${items.map(pdDistRowHtml).join('')}</div>
