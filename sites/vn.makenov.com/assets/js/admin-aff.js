...
async function affMarketersView(){
  ...
  <div class="tbl-wrap"><table><thead><tr><th>코드</th><th>이름</th><th>연락처</th><th>클릭 30일</th><th>리드(대기)</th><th>전환율</th><th>승인 금액</th><th>잔액</th><th>상태</th><th style="width:180px"></th></tr></thead><tbody>
  ${rows.length ? rows.map(m => `<tr class="row-hover"><td><b>${esc(m.code)}</b><div class="sub">${esc(String(m.created_at || '').slice(0, 10))}</div></td><td>${esc(m.name)}<div class="sub">${esc(m.email)}</div></td><td>${esc(m.phone || '')}<div class="sub">Zalo ${esc(m.zalo || '-')}</div></td><td><b>${m.clicks30}</b></td><td>${m.leads} (${m.pending})</td><td>${m.conv == null ? '-' : Math.round(m.conv * 100) + '%'}</td><td>${vndK(m.approved_vnd)}</td><td><b>${vndK(m.balance_vnd)}</b></td><td>${affBadge(m.status)}</td>
