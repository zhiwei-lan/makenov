...
async function affMarketersView(){
  ...
  ${rows.length ? rows.map(m => `<tr class="row-hover"><td><b>${esc(m.code)}</b><div class="sub">${esc(String(m.created_at || '').slice(0, 10))}</div></td><td>${esc(m.name)}<div class="sub">${esc(m.email)}</div></td><td>${esc(m.phone || '')}<div class="sub">Zalo ${esc(m.zalo || '-')}</div>${(m.channels || []).map(u => `<div class="sub"><a href="${esc(u)}" target="_blank" rel="noopener">${esc(u.replace(/^https?:\/\/(www\.)?/, '').slice(0, 40))} ↗</a></div>`).join('')}</td><td><b>${m.clicks30}</b></td><td>${m.leads} (${m.pending})</td><td>${m.approval_rate == null ? '<span class="sub">3건 미만</span>' : `<b style="color:${m.approval_rate >= .8 ? '#13A583' : m.approval_rate >= .5 ? '#B76E00' : '#D9534F'}">${Math.round(m.approval_rate * 100)}%</b>`}</td><td>${m.conv == null ? '-' : Math.round(m.conv * 100) + '%'}</td><td>${vndK(m.approved_vnd)}</td><td><b>${vndK(m.balance_vnd)}</b></td><td>${affBadge(m.status)}</td>
...
async function affMarketerDetailView(id){
  ...
  return `<div class="bar" style="margin:0 0 12px"><h3 style="margin:0">${esc(m.name)} <span style="color:var(--adm-sub);font-weight:500;font-size:13px">코드 ${esc(m.code)} · ${esc(m.email)} · ${esc(m.phone || '')} · Zalo ${esc(m.zalo || '-')}${(m.channels || []).map((u, i) => ` · <a href="${esc(u)}" target="_blank" rel="noopener">채널${i + 1} ↗</a>`).join('')}</span></h3><span class="grow"></span><button class="btn btn-ghost btn-sm" onclick="affDetailId=null;renderAff()">목록</button></div>
...
async function affDeleteDemo(){
  if(!confirm('데모 마케터 8명과 그 리드·클릭·출금을 모두 지웁니다. 되돌릴 수 없습니다.')) return;
